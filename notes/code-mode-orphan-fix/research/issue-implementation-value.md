...
| [#35613](https://github.com/openai/codex/issues/35613) — code-mode live session IDs | 28/30 · A | **4/5 · WORKING / PATCH-READY** | Includes an executable negative reproduction, an exploratory implementation and tests, exact data-flow analysis, and a narrow provenance-based design: retain creator `CellId` on process entries and report exact-cell live IDs. The remaining questions are explicit rather than hidden. The linked prototype branch was not independently resolved during this pass, so this is not rated 5/5. |
