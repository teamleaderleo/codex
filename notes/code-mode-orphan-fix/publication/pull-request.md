...
The simplest correct implementation reports the complete per-cell live list. The manager nominally prunes the whole store at 64 entries, and the IDs are short numeric values, so even a per-cell list at that scale costs only a few hundred bytes.
