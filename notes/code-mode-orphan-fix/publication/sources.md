...
This index separates primary sources from secondary synthesis. A link appearing here does not mean every public issue or pull request should include it. The [issue](issue.md) cites only the sources needed to verify the problem and expected behaviour. The [pull request](pull-request.md) cites the implementation, tests, conventions, and bounded validation. The [deep dive](deep-dive.md) carries the fuller technical and historical record.
...
2. **Executable records and exact comparisons** — strongest source for what was run and observed.
...
| [Final comparison](https://github.com/teamleaderleo/codex/compare/61a44880a85d2fd0d8770908dea5733495e571c8...760216784efaee1ba6a3b1250349f31d5f91c7ca) | Complete public code and test diff. |
...
Private validation records preserve the exact commands, focused results, compatibility repetitions, and matched broad-suite classification. Machine-specific setup and raw troubleshooting are not required for public patch review and remain private by default.
...
The commit-pinned test sources above establish what was exercised. The execution claims are a public summary of retained validation records; raw logs remain private unless a maintainer question makes a scrubbed excerpt materially useful.
...
| [Tool-system refactor PR #4510](https://github.com/openai/codex/pull/4510) / [commit `33d3ecbc`](https://github.com/openai/codex/commit/33d3ecbccca4b92cfb2a77002387de30302f337f) | Established handlers, routing, registry, and shared invocation boundaries. |
...
- [openai/codex#14731](https://github.com/openai/codex/issues/14731), [#15723](https://github.com/openai/codex/issues/15723), and [#32188](https://github.com/openai/codex/issues/32188) — broader completion blocking, wake-up, and lifecycle or eventing questions.
...
- The [pull-request alternatives](pull-request.md#alternatives-considered) and [technical deep dive](deep-dive.md#alternatives-considered) summarise the reviewed alternatives and clearly separate rejection from deferral.

## Secondary synthesis and methodology

The deep dive is synthesis rather than primary evidence. Its concrete claims link through to commit-pinned code, tests, comparisons, upstream history, or clearly labelled interpretation in this index.

Independent static review also raised test-convention, cleanup, compatibility, cardinality, and maintainability questions. It did not compile or run the candidate. Findings that changed the final tests were subsequently checked through repository-native execution.
...
- unsanitised one-off analysis scripts; and
- raw validation logs or launcher troubleshooting.
